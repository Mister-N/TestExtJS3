 ...
 */
Test.QualificationGrid = function(config) {
    config = config || {};
    config.id = config.id || 'test-grid-qualification';
    Ext.applyIf(config, {
        title: 'Образование',
        viewConfig: {
            forceFit: true,
            ...
            getRowClass: function (rec, ri, p) {
                return !rec.data.active
                    ? 'test-qualification-row-disabled'
                    : '';
            }
        },
        ...
        listeners: {
            rowDblClick: function (grid, rowIndex, e) {
                ...
                this.updateQualification(grid, e, row);
            }
        },
        paging: true,
        remoteSort: true,
    ...
    Test.QualificationGrid.superclass.constructor.call(this,config);
    this.config = config;
};
Ext.extend(Test.QualificationGrid,Test.grid.Grid,{
    windows: {},
    ...
    getTopBar: function (config) {
        console.log('q-toolBar',config.id);
        return [{
    ...
    createItem: function (btn, e) {
        var w = Ext.ComponentMgr.create({
            xtype: 'test-qualification-window-create',
            id: Ext.id(),
            url : this.config.url,
            listeners: {
                success: {
    ...
    updateQualification: function (btn, e, row) {
    ...
    removeQualification: function (act, btn, e) {
        var ids = this._getSelectedIds();
        if (!ids.length) {
            return false;
        }
...
Ext.reg('test-qualification-grid', Test.QualificationGrid);